...
open class GenPagesTemplateLongList2LongList2 : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesTemplateLongList2LongList2) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesTemplateLongList2LongList2
            val _cache = __ins.renderCache
            fun gen_lerpNumber_fn(value1: Number, value2: Number, amount: Number): Number {
                return (value1 + (value2 - value1) * amount)
            }
            val lerpNumber = ::gen_lerpNumber_fn
            val swiperList = ref(_uA<SwiperViewItem__4>(SwiperViewItem__4(type = "UpdatedDate", name = "最新上架", id = "list-id-1", preload = true), SwiperViewItem__4(type = "FreeHot", name = "免费热榜", id = "list-id-2", preload = false), SwiperViewItem__4(type = "PaymentHot", name = "付费热榜", id = "list-id-3", preload = false), SwiperViewItem__4(type = "HotList", name = "热门总榜", id = "list-id-4", preload = false)))
            ...
            val swiperTabsRect = ref(_uA<SwiperTabsItem__3>())
                ...
                return wrapUTSPromise(suspend {
                        ...
                        run {
                            ...
                            while(i < tabs.length){
                                ...
                                swiperTabsRect.value.push(SwiperTabsItem__3(x = x, w = w))
                                i++
                            }
                        }
                })
            ...
            return fun(): Any? {
                return _cE("scroll-view", _uM("ref_key" to "pageScrollView", "ref" to pageScrollView, "class" to "page", "bounces" to false, "onStartnestedscroll" to onStartNestedScroll, "onNestedprescroll" to onNestedPreScroll, "nested-scroll-child" to unref(nestedScrollChildId)), _uA(
                        ...
                        _cE("swiper", _uM("ref_key" to "swiper", "ref" to swiper, "class" to "swiper-view", "current" to unref(swiperIndex), "onTransition" to onSwiperTransition, "onAnimationfinish" to onSwiperAnimationfinish), _uA(
                            _cE(Fragment, null, RenderHelpers.renderList(unref(swiperList), fun(item, index, __index, _cached): Any {
                                return _cE("swiper-item", _uM("class" to "swiper-item", "key" to index), _uA(
                                    _cV(unref(GenPagesTemplateLongListLongListPageClass), _uM("ref_for" to true, "ref_key" to "longPageRef", "ref" to longPageRef, "id" to item.id, "type" to item.type, "preload" to item.preload), null, 8, _uA(
                                        "id",
                                        "type",
                                        "preload"
                                    ))
