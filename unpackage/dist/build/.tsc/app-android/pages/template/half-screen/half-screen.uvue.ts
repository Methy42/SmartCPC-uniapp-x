...
const _cache = this.$.renderCache
  return _cE("view", _uM({
    ...
    _cE("text", _uM({ class: "tip" }), "半屏弹窗，演示了弹出层内scroll-view滚动到顶时由滚变拖。本效果是通过监听TouchEvent实现，当半屏窗口移动时禁用scroll-view的滚动，避免两者的冲突。"),
    _cE("button", _uM({
      class: "bottomButton",
    ...
    _cE("view", _uM({
      ...
      _cE("view", _uM({ class: "halfTitle" }), "半屏弹窗标题"),
      _cE("scroll-view", _uM({
        ref: "halfScroll",
        class: "halfScroll",
        bounces: "true",
        ...
        _cE(Fragment, null, RenderHelpers.renderList(_ctx.list, (item, index, __index, _cached): any => {
          return _cE("view", _uM({
            key: index,
            class: "item"
          }), " half screen content-" + _tD(item), 1 /* TEXT */)
        }), 128 /* KEYED_FRAGMENT */)
      ], 8 /* PROPS */, ["direction"])
    ], 40 /* PROPS, NEED_HYDRATION */, ["onTouchstart", "onTouchmove", "onTouchend"])
  ], 512 /* NEED_PATCH */)
}
const GenPagesTemplateHalfScreenHalfScreenStyles = [_uM([["page", _pS(_uM([["flexGrow", 1], ["flexShrink", 1], ["flexBasis", "0%"], ["backgroundColor", "#A9A9A9"]]))], ["tip", _pS(_uM([["marginTop", 10], ["marginRight", 10], ["marginBottom", 10], ["marginLeft", 10]]))], ["bottomButton", _pS(_uM([["position", "absolute"], ["width", "100%"], ["bottom", 0], ["paddingBottom", "env(safe-area-inset-bottom,0px)"]]))], ["halfScreen", _pS(_uM([["position", "absolute"], ["top", "100%"], ["width", "100%"], ["height", "70%"], ["transitionTimingFunction", "ease-in-out"], ["transitionProperty", "top"], ["transitionDuration", "0ms"], ["visibility", "hidden"]]))], ["halfTitle", _pS(_uM([["alignItems", "center"], ["justifyContent", "center"], ["height", 48], ["backgroundColor", "#F8F8FF"], ["borderTopLeftRadius", 10], ["borderTopRightRadius", 10], ["borderBottomRightRadius", 0], ["borderBottomLeftRadius", 0]]))], ["halfScroll", _pS(_uM([["backgroundColor", "#FFFFFF"], ["flexGrow", 1], ["flexShrink", 1], ["flexBasis", "0%"]]))], ["item", _pS(_uM([["height", 100]]))], ["@TRANSITION", _uM([["halfScreen", _uM([["timingFunction", "ease-in-out"], ["property", "top"], ["duration", "0ms"]])]])]])]
