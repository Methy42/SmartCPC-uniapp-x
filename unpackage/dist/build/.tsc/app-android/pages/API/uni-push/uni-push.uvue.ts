...
return (): any | null => {
  ...
  return _cE("scroll-view", _uM({
    ...
    _cE("view", null, [
      ...
      _cE("text", _uM({ class: "instructions" }), " 不同手机厂商的角标显示规则不同，有部分设备的rom版本不支持显示角标，另有部分rom需要在应用的通知管理里开启`桌面角标`配置，才可以设置角标成功。\n 部分rom需要在设置中同时开启`通知开关`和`桌面角标`配置，才允许设置角标，例如鸿蒙4.2。 \n 另外针对高版本小米设备，会借助创建通知栏消息来设置角标数，所以设置时需要注意是否有权限创建通知栏消息。 "),
      _cE("button", _uM({
        class: "normal-button",
        type: "default",
...
const GenPagesAPIUniPushUniPushStyles = [_uM([["normal-button", _pS(_uM([["width", "100%"]]))], ["instructions", _pS(_uM([["marginTop", 10], ["marginLeft", 10], ["marginRight", 10], ["backgroundColor", "#eeeeee"]]))]])]
