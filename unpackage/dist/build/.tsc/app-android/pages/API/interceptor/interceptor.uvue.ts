...
const _component_navigator = resolveComponent("navigator")
  ...
  return _cE("view", _uM({
    ...
    _cE("button", _uM({ onClick: _ctx.addInterceptor }), "添加路由拦截器", 8 /* PROPS */, ["onClick"]),
    _cE("button", _uM({ onClick: _ctx.removeInterceptor }), "移除路由拦截器", 8 /* PROPS */, ["onClick"]),
    _cE("text", null, "点击下方按钮" + _tD(_ctx.msg), 1 /* TEXT */),
    _cE("button", _uM({ onClick: _ctx.navigateTo }), "navigatorTo API跳转到测试页面", 8 /* PROPS */, ["onClick"]),
    _cV(_component_navigator, _uM({ url: "./page1" }), _uM({
      default: withSlotCtx((): any[] => [
        _cE("button", _uM({ class: "navigatorButton" }), "navigator组件跳转到测试页面")
      ]),
      _: 1 /* STABLE */
    })),
    _cE("button", _uM({ onClick: _ctx.addSwitchTabInterceptor }), "添加switchTab拦截器", 8 /* PROPS */, ["onClick"]),
    _cE("button", _uM({ onClick: _ctx.removeSwitchTabInterceptor }), "移除switchTab拦截器", 8 /* PROPS */, ["onClick"]),
    _cE("button", _uM({ onClick: _ctx.switchTab }), "switchTab API", 8 /* PROPS */, ["onClick"])
  ], 4 /* STYLE */)
}
