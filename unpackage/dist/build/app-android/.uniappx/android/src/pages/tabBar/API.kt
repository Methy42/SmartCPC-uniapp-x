...
open class GenPagesTabBarAPI : BasePage {
            ...
            _cE("view", _uM("class" to _nC(_uA(
                ...
                if (isTrue(!_ctx.hasLeftWin)) {
                    _cE("view", _uM("key" to 1, "class" to "uni-text-box"), _uA(
                        ...
                        _cV(_component_u_link, _uM("href" to "https://uniapp.dcloud.io/uni-app-x/api/", "text" to "https://uniapp.dcloud.io/uni-app-x/api/", "inWhiteList" to true), null, 8, _uA(
                            "href",
                            "text"
                        ))
                    ))
                                        ...
                                        return _cE(Fragment, _uM("key" to childMenuItem.id), _uA(
                                            if (childMenuItem.items.length == 0) {
                                                _cE("view", _uM("key" to 0, "class" to _nC(_uA(
                                                    "pl",
                                                    _uM("uni-navigate-item" to (childMenuItem.path !== "set-tab-bar" || _ctx.windowWidth <= 768))
                                                )), "hover-class" to "is--active", "onClick" to fun(){
                                                    _ctx.goPage("/" + childMenuItem.path)
                                                }), _uA(
                                                    if (isTrue(childMenuItem.path !== "set-tab-bar" || _ctx.windowWidth <= 768)) {
                ...
                if (isTrue(!_ctx.hasLeftWin)) {
                    _cE("view", _uM("key" to 2, "ref" to "pop", "onClick" to fun(){
                        _ctx.hidePop()
                    }, "class" to "popup"), _uA(
                        _cE("view", _uM("style" to _nS(_uM("width" to "90%", "background-color" to "white")), "onClick" to withModifiers(_ctx.stopClickPop, _uA(
                            "stop"
                        ))), _uA(
    ...
    override fun data(): Map<String, Any?> {
        return _uM("menu" to menu__1 as UTSArray<MenuItem>, "arrowRightIcon" to "/static/icons/arrow-right.png", "hasLeftWin" to computed<Boolean>(fun(): Boolean {
            return !state.noMatchLeftWindow
        }
        ), "leftWinActive" to computed<String>(fun(): String {
            return state.leftWinActive.slice(1)
        }
        ), "windowWidth" to computed<Number>(fun(): Number {
    ...
    open var showPop = ::gen_showPop_fn
    open fun gen_showPop_fn() {
        (this.`$refs`["pop"] as UniElement).style.setProperty("display", "flex")
    }
    open var hidePop = ::gen_hidePop_fn
    open fun gen_hidePop_fn() {
        (this.`$refs`["pop"] as UniElement).style.setProperty("display", "none")
    }
    open var stopClickPop = ::gen_stopClickPop_fn
    open fun gen_stopClickPop_fn(e: PointerEvent) {
        e.stopPropagation()
    }
    open var stopPropagation = ::gen_stopPropagation_fn
    open fun gen_stopPropagation_fn() {}
    companion object {
