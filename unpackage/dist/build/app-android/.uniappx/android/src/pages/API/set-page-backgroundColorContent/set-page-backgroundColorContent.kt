...
open class GenPagesAPISetPageBackgroundColorContentSetPageBackgroundColorContent : BasePage {
    ...
    override fun `$render`(): Any? {
        ...
        return _cE("view", _uM("class" to "container", "onClick" to _ctx.changeColor), _uA(
    ...
    override fun data(): Map<String, Any?> {
        return _uM("isChange" to false, "currentBackgroundColorContent" to "" as Any?)
    }
    open var changeColor = ::gen_changeColor_fn
    open fun gen_changeColor_fn() {
        var pages = getCurrentPages()
        var page = pages[pages.length - 1]
        page.setPageStyle(_uO("backgroundColorContent" to if (this.isChange) {
            ""
        } else {
            "red"
        }
