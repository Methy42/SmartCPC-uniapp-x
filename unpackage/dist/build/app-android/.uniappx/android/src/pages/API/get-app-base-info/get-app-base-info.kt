...
open class GenPagesAPIGetAppBaseInfoGetAppBaseInfo : BasePage {
            ...
            _cE("view", _uM("class" to "uni-common-mt"), _uA(
                ...
                _cE("view", _uM("class" to "uni-padding-wrap"), _uA(
                    _cE("view", _uM("class" to "uni-btn-v"), _uA(
                        _cE("button", _uM("type" to "primary", "onClick" to _ctx.getAppBaseInfo), "获取App基础信息", 8, _uA(
                            "onClick"
                        ))
                    ))
                ))
    ...
    open var items: UTSArray<Item__4> by `$data`
    @Suppress("USELESS_CAST")
    override fun data(): Map<String, Any?> {
        return _uM("title" to "getAppBaseInfo", "items" to _uA<Item__4>())
    }
    ...
    open fun gen_getAppBaseInfo_fn() {
        ...
        this.items = _uA<Item__4>()
        keys.sort().forEach(fun(key){
            val value = res[key]
            if (value != null) {
                val item = Item__4(label = key, value = "" + (if ((UTSAndroid.`typeof`(value) == "object")) {
