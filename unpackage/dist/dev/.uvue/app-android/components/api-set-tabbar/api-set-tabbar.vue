    ...
    setup(__props) {
        ...
        return (): any | null => {
            ...
            return _cE("view", _uM({ class: "uni-padding-wrap" }), [
                ...
                _cE("button", _uM({
                    class: "button",
                    onClick: setTabBarBadge
                }), _tD(!unref(hasSetTabBarBadge) ? '设置tab徽标' : '移除tab徽标'), 1 /* TEXT */),
                _cE("button", _uM({
                    class: "button",
                    onClick: showTabBarRedDot
                }), _tD(!unref(hasShownTabBarRedDot) ? '显示红点' : '移除红点'), 1 /* TEXT */),
                _cE("button", _uM({
                    class: "button",
                    onClick: customStyle
                }), _tD(!unref(hasCustomedStyle) ? '自定义Tab样式' : '移除自定义样式'), 1 /* TEXT */),
                _cE("button", _uM({
                    class: "button",
                    onClick: customItem
                }), _tD(!unref(hasCustomedItem) ? '自定义Tab信息' : '移除自定义信息'), 1 /* TEXT */),
                _cE("button", _uM({
                    class: "button",
                    onClick: hideTabBar
                }), _tD(!unref(hasHiddenTabBar) ? '隐藏TabBar' : '显示TabBar'), 1 /* TEXT */),
                _cE("button", _uM({
                    class: "button",
                    onClick: hideTabBarItem
                }), _tD(!unref(hasHiddenTabBarItem) ? '隐藏接口Item' : '显示接口Item'), 1 /* TEXT */),
                _cE("button", _uM({
                    class: "button",
                    onClick: setTabBarTitle
                }), _tD(!unref(hasSetLongTitle) ? '自定义超长标题' : '移除自定义信息'), 1 /* TEXT */),
                _cE("view", _uM({ class: "btn-area" }))
            ]);
        };
    }
});
...
const GenComponentsApiSetTabbarApiSetTabbarStyles = [_uM([["button", _pS(_uM([["marginTop", 15], ["marginLeft", 0], ["marginRight", 0]]))], ["btn-area", _pS(_uM([["paddingTop", 15]]))]])];
