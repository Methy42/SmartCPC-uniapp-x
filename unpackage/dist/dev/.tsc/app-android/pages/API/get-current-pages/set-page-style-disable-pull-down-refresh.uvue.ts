
  class Page {
    constructor(public route : string) {
    }
  }

  const __sfc__ = defineComponent({
    data() {
      return {
        checked: false,
        pages: [] as Page[],
        currentPageStyle: {} as UTSJSONObject,
        // TODO
        enablePullDownRefreshStatus: true
      }
    },
    computed: {
      pageStyleText() : string {
        return JSON.stringify(this.currentPageStyle)
      }
    },
    onLoad() {
      const pages = getCurrentPages();
      const currentPage = pages[pages.length - 1];
      this.currentPageStyle = currentPage.getPageStyle();
      this.enablePullDownRefreshStatus = this.currentPageStyle["enablePullDownRefresh"] as boolean
    },
    onPullDownRefresh() {
      setTimeout(() => {
        uni.stopPullDownRefresh()
      }, 3000)
    },
    methods: {
      getPageStyle() {
        const pages = getCurrentPages();
        const currentPage = pages[pages.length - 1];
        this.currentPageStyle = currentPage.getPageStyle();
      },
      setPageStyle(enable : boolean) {
        // 目前仅支持 enablePullDownRefresh
        const pages = getCurrentPages();
        const currentPage = pages[pages.length - 1];
        currentPage.setPageStyle({
          enablePullDownRefresh: enable
        });
        this.enablePullDownRefreshStatus = enable
      },
      startPullDownRefresh() {
        uni.startPullDownRefresh()
      }
    },
  })

export default __sfc__
function GenPagesAPIGetCurrentPagesSetPageStyleDisablePullDownRefreshRender(this: InstanceType<typeof __sfc__>): any | null {
const _ctx = this
const _cache = this.$.renderCache
const _component_page_head = resolveEasyComponent("page-head",_easycom_page_head)

  return _cE(Fragment, null, [
    _cV(_component_page_head, _uM({ title: "getCurrentPages" })),
    _cE("view", _uM({ class: "uni-padding-wrap" }), [
      _cE("button", _uM({
        class: "btn btn-get-page-style",
        type: "default",
        onClick: _ctx.getPageStyle
      }), "getPageStyle", 8 /* PROPS */, ["onClick"]),
      _cE("button", _uM({
        class: "btn btn-set-page-style-1",
        type: "default",
        onClick: () => {_ctx.setPageStyle(true)}
      }), "setPageStyle(true)", 8 /* PROPS */, ["onClick"]),
      _cE("button", _uM({
        class: "btn btn-set-page-style-0",
        type: "default",
        onClick: () => {_ctx.setPageStyle(false)}
      }), "setPageStyle(false)", 8 /* PROPS */, ["onClick"]),
      _cE("text", _uM({ class: "page-style" }), "当前 PageStyle"),
      _cE("text", _uM({ class: "page-style-value" }), _tD(_ctx.pageStyleText), 1 /* TEXT */),
      _cE("text", _uM({ class: "status" }), "状态："),
      _cE("view", _uM({ class: "status-list" }), [
        _cE("text", null, "enablePullDownRefresh: " + _tD(_ctx.enablePullDownRefreshStatus), 1 /* TEXT */)
      ]),
      _cE("text", _uM({ class: "tips" }), "当前版本仅支持设置属性 enablePullDownRefresh")
    ])
  ], 64 /* STABLE_FRAGMENT */)
}
const GenPagesAPIGetCurrentPagesSetPageStyleDisablePullDownRefreshStyles = [_uM([["btn", _pS(_uM([["marginTop", 10]]))], ["page-style", _pS(_uM([["marginTop", 15]]))], ["page-style-value", _pS(_uM([["marginTop", 5], ["paddingTop", 5], ["paddingRight", 5], ["paddingBottom", 5], ["paddingLeft", 5], ["backgroundColor", "#ffffff"], ["width", "100%"]]))], ["status", _pS(_uM([["marginTop", 20]]))], ["status-list", _pS(_uM([["marginTop", 5]]))], ["tips", _pS(_uM([["fontSize", 12], ["marginTop", 15], ["opacity", 0.8]]))]])]

import _easycom_page_head from '@/components/page-head/page-head.vue'
