...
return (): any | null => {
  ...
  return _cE("view", _uM({
    style: _nS(_uM({"flex":"1","background-color":"aliceblue"}))
  }), [
    _cE("view", _uM({ class: "tips" }), "list-view组件虽然在渲染层有recycle机制，但长列表的vnode/Element太多也会造成初始化卡顿。 本示例有2000条数据，通过分批创建列表项，减少初始化卡顿。并通过闲时创建机制避免影响列表滚动等UI操作"),
    _cE("list-view", _uM({
      ...
      _cE(Fragment, null, RenderHelpers.renderList(unref(list), (item, index, __index, _cached): any => {
        return _cE("list-item", _uM({
          ...
          _cE("view", null, [
            _cE("text", _uM({
              style: _nS(_uM({"font-size":"12px","color":"#999999"}))
            }), _tD(item.info), 5 /* TEXT, STYLE */)
          ])
        ])
      }), 128 /* KEYED_FRAGMENT */)
...
const GenPagesTemplateLongListBatchLongListBatchStyles = [_uM([["tips", _pS(_uM([["marginTop", 10], ["marginRight", 10], ["marginBottom", 10], ["marginLeft", 10], ["borderTopLeftRadius", 5], ["borderTopRightRadius", 5], ["borderBottomRightRadius", 5], ["borderBottomLeftRadius", 5], ["paddingTop", 10], ["paddingRight", 10], ["paddingBottom", 10], ["paddingLeft", 10], ["backgroundColor", "#FFFFFF"]]))], ["item", _pS(_uM([["marginTop", 5], ["marginRight", 10], ["marginBottom", 5], ["marginLeft", 10], ["paddingTop", 10], ["paddingRight", 10], ["paddingBottom", 10], ["paddingLeft", 10], ["borderTopLeftRadius", 5], ["borderTopRightRadius", 5], ["borderBottomRightRadius", 5], ["borderBottomLeftRadius", 5], ["backgroundColor", "#FFFFFF"]]))]])]
