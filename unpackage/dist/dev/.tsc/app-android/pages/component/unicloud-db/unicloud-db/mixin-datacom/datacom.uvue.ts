...
const _cache = this.$.renderCache
  return _cE("view", null, [
    isTrue(_ctx.mixinDatacomLoading)
      ? _cE("view", _uM({ key: 0 }), "Loading...")
      : isTrue(_ctx.mixinDatacomError)
        ? _cE("view", _uM({ key: 1 }), " 请求错误：" + _tD(_ctx.mixinDatacomError), 1 /* TEXT */)
        : _cE("view", _uM({ key: 2 }), [
            _cE("scroll-view", _uM({ class: "list-view" }), [
              _cE(Fragment, null, RenderHelpers.renderList(_ctx.mixinDatacomResData, (item, _, __index, _cached): any => {
                return _cE("view", _uM({ class: "list-item" }), [
                  _cE("text", null, _tD(item), 1 /* TEXT */)
                ])
              }), 256 /* UNKEYED_FRAGMENT */)
            ])
          ]),
    isTrue(!_ctx.mixinDatacomLoading && _ctx.mixinDatacomResData.length==0)
      ? _cE("view", _uM({ key: 3 }), " 无数据 ")
      : _cC("v-if", true)
  ])
}
