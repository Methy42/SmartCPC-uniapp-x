
  import { state, setEventCallbackNum } from '@/store/index.uts'
  const __sfc__ = defineComponent({
    data() {
      // 20180112 HBuilderX内测开始 :)
      const _years : number[] = []
      const _year = 2018
      const _months : number[] = []
      const _month : number = 1
      const _days : number[] = []
      const _day = 12
      for (let i = 2000; i <= _year; i++) {
        _years.push(i)
      }
      for (let i = 1; i <= 12; i++) {
        _months.push(i)
      }
      for (let i = 1; i <= 31; i++) {
        _days.push(i)
      }
      return {
        title: 'picker-view',
        years: _years as number[],
        year: _year as number,
        months: _months as number[],
        month: _month as number,
        days: _days as number[],
        day: _day as number,
        value: [_year - 2000, _month - 1, _day - 1] as number[],
        result: [] as number[],
        indicatorStyle: 'height: 50px;',
        indicatorClass: '',
        maskStyle: '',
        maskClass: '',
        maskTopStyle: '',
        maskBottomStyle: ''
      }
    },
    methods: {
      setIndicatorStyle(checked : boolean) {
        const extraStyle = 'height: 50px;border:#ff5500 solid 1px;background:rgba(182, 179, 255, 0.4);';




        this.indicatorStyle = checked ? extraStyle : "height: 50px;";

      },
      setIndicatorClass(checked : boolean) {
        this.indicatorClass = checked ? "indicator-test" : ""
      },
      setMaskStyle(checked : boolean) {
        const extraMaskStyle = "background-image: linear-gradient(to bottom, #d8e5ff, rgba(216, 229, 255, 0));"
        this.maskStyle = checked ? extraMaskStyle : ""
      },
      setMaskClass(checked : boolean) {
        this.maskClass = checked ? "mask-test" : ""
      },
      setMaskTopStyle(checked : boolean) {
        const linearToTop = "background-image: linear-gradient(to bottom, #f4ff73, rgba(216, 229, 255, 0));"




        this.maskTopStyle = checked ? linearToTop : ""

      },
      setMaskBottomStyle(checked : boolean) {
        const linearToBottom = "background-image: linear-gradient(to top, #f4ff73, rgba(216, 229, 255, 0));"




        this.maskBottomStyle = checked ? linearToBottom : ""

      },
      // 自动化测试
      getEventCallbackNum() : number {
        return state.eventCallbackNum
      },
      // 自动化测试
      setEventCallbackNum(num : number) {
        setEventCallbackNum(num)
      },
      bindChange(e : UniPickerViewChangeEvent) {
        // 自动化测试 触发事件元素、type 类型
        // console.log(e.target?.tagName, e.type);
        if ((e.target?.tagName ?? '').includes('PICKER-VIEW')) {
          this.setEventCallbackNum(state.eventCallbackNum + 1)
        }
        if (e.type === 'change') {
          this.setEventCallbackNum(state.eventCallbackNum + 2)
        }
        const val = e.detail.value
        this.result = val
        this.year = this.years[val[0]]
        this.month = this.months[val[1]]
        this.day = this.days[val[2]]
      },
      setValue() {
        this.value = [0, 1, 30] as number[]
      },
      setValue1() {
        this.value = [10, 10, 10] as number[]
      },
    }
  })

export default __sfc__
function GenPagesComponentPickerViewPickerViewRender(this: InstanceType<typeof __sfc__>): any | null {
const _ctx = this
const _cache = this.$.renderCache
const _component_page_head = resolveEasyComponent("page-head",_easycom_page_head)
const _component_picker_view_column = resolveComponent("picker-view-column")
const _component_picker_view = resolveComponent("picker-view", true)
const _component_boolean_data = resolveEasyComponent("boolean-data",_easycom_boolean_data)

  return _cE("view", null, [
    _cV(_component_page_head, _uM({ title: _ctx.title }), null, 8 /* PROPS */, ["title"]),
    _cE("view", _uM({ class: "uni-padding-wrap" }), [
      _cE("view", _uM({ class: "uni-title" }), " 日期：" + _tD(_ctx.year) + "年" + _tD(_ctx.month) + "月" + _tD(_ctx.day) + "日 ", 1 /* TEXT */)
    ]),
    _cV(_component_picker_view, _uM({
      class: "picker-view",
      value: _ctx.value,
      onChange: _ctx.bindChange,
      "indicator-style": _ctx.indicatorStyle,
      "indicator-class": _ctx.indicatorClass,
      "mask-style": _ctx.maskStyle,
      "mask-class": _ctx.maskClass,
      "mask-top-style": _ctx.maskTopStyle,
      "mask-bottom-style": _ctx.maskBottomStyle
    }), _uM({
      default: withSlotCtx((): any[] => [
        _cV(_component_picker_view_column, _uM({ class: "picker-view-column" }), _uM({
          default: withSlotCtx((): any[] => [
            _cE(Fragment, null, RenderHelpers.renderList(_ctx.years, (item, index, __index, _cached): any => {
              return _cE("view", _uM({
                class: "item",
                key: index
              }), [
                _cE("text", _uM({ class: "text" }), _tD(item) + "年", 1 /* TEXT */)
              ])
            }), 128 /* KEYED_FRAGMENT */)
          ]),
          _: 1 /* STABLE */
        })),
        _cV(_component_picker_view_column, _uM({ class: "picker-view-column" }), _uM({
          default: withSlotCtx((): any[] => [
            _cE(Fragment, null, RenderHelpers.renderList(_ctx.months, (item, index, __index, _cached): any => {
              return _cE("view", _uM({
                class: "item",
                key: index
              }), [
                _cE("text", _uM({ class: "text" }), _tD(item) + "月", 1 /* TEXT */)
              ])
            }), 128 /* KEYED_FRAGMENT */)
          ]),
          _: 1 /* STABLE */
        })),
        _cV(_component_picker_view_column, _uM({ class: "picker-view-column" }), _uM({
          default: withSlotCtx((): any[] => [
            _cE(Fragment, null, RenderHelpers.renderList(_ctx.days, (item, index, __index, _cached): any => {
              return _cE("view", _uM({
                class: "item",
                key: index
              }), [
                _cE("text", _uM({ class: "text" }), _tD(item) + "日", 1 /* TEXT */)
              ])
            }), 128 /* KEYED_FRAGMENT */)
          ]),
          _: 1 /* STABLE */
        }))
      ]),
      _: 1 /* STABLE */
    }), 8 /* PROPS */, ["value", "onChange", "indicator-style", "indicator-class", "mask-style", "mask-class", "mask-top-style", "mask-bottom-style"]),
    _cV(_component_boolean_data, _uM({
      defaultValue: false,
      title: "设置选择器中间选中框的样式",
      onChange: _ctx.setIndicatorStyle
    }), null, 8 /* PROPS */, ["onChange"]),
    _cV(_component_boolean_data, _uM({
      defaultValue: false,
      title: "设置蒙层上半部分的样式",
      onChange: _ctx.setMaskTopStyle
    }), null, 8 /* PROPS */, ["onChange"]),
    _cV(_component_boolean_data, _uM({
      defaultValue: false,
      title: "设置蒙层下半部分的样式",
      onChange: _ctx.setMaskBottomStyle
    }), null, 8 /* PROPS */, ["onChange"])
  ])
}
const GenPagesComponentPickerViewPickerViewStyles = [_uM([["picker-view", _pS(_uM([["width", "100%"], ["height", 320], ["marginTop", 10], ["marginBottom", 20]]))], ["item", _pS(_uM([["height", 50]]))], ["text", _pS(_uM([["lineHeight", "50px"], ["textAlign", "center"]]))], ["indicator-test", _pS(_uM([["height", 50], ["borderTopWidth", 1], ["borderRightWidth", 1], ["borderBottomWidth", 1], ["borderLeftWidth", 1], ["borderTopStyle", "solid"], ["borderRightStyle", "solid"], ["borderBottomStyle", "solid"], ["borderLeftStyle", "solid"], ["borderTopColor", "#ff5500"], ["borderRightColor", "#ff5500"], ["borderBottomColor", "#ff5500"], ["borderLeftColor", "#ff5500"], ["backgroundImage", "none"], ["backgroundColor", "rgba(182,179,255,0.4)"]]))], ["mask-test", _pS(_uM([["backgroundImage", "linear-gradient(to bottom, #d8e5ff, rgba(216, 229, 255, 0))"]]))]])]

import _easycom_page_head from '@/components/page-head/page-head.vue'
import _easycom_boolean_data from '@/components/boolean-data/boolean-data.vue'
